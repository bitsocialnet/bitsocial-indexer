...
- Plan/schema errors, absent/ambiguous controls, changed refs, uncertain choices, API failures, step/time/budget limits, assertion failures, and cleanup failures return `incomplete`. Unsupported snapshot serialization is unavailable, never guessed. The coding agent can inspect the page and continue with ordinary Playwright.
